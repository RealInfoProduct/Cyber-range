...
import { faUser, faEdit, faTrash,faShoppingCart, faPaperPlane, faDownload } from '@fortawesome/free-solid-svg-icons';
...
@Component({
  selector: 'app-viewpurchasevoucherlist',
  templateUrl: './viewpurchasevoucherlist.component.html',
  styleUrls: ['./viewpurchasevoucherlist.component.css']
})
export class ViewpurchasevoucherlistComponent implements OnInit {
  ...
  current_url_array = [];
    ...
    voucherlist()
    {
      /* user data table */
		   ...
		   // console.log(startNumber);
			...
			that.http
			  .post<DataTablesResponse>(
				this.serverUrl+'datatable-api/get-purchase-voucher-list',
				params, {}
			  ).subscribe(resp => {
		  ...
		  columns: [{ data: 's_no' }, { data: 'voucher_name' },  { data: 'price' },  { data: 'credit' },  { data: 'order_id' },  { data: 'trans_id' }, { data: 'datetime' }]
		};
    }
	...
	downLoad(order_id:any)
	{
		const formData = new FormData();
		formData.append('order_id',order_id);
		var api = 'candidate-api/download-payment-pdf';
		this.BackenddbService.postData(api,formData).subscribe((res:any) => {
			console.log(res);
			if(res.status=="success")
			{
				...
				//const blob = new Blob([res.url], {type: "application/pdf"});
				 //saveAs(blob, "bill.pdf");

				//var file = new Blob([res.url], { type: 'application/pdf' })
				//var fileURL = URL.createObjectURL(file);
				//window.open(fileURL); // if you want to open it in new tab
				// const objectUrl = URL.createObjectURL(res.url)
