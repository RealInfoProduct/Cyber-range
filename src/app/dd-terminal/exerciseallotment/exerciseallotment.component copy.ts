...
export class ExerciseallotmentComponent implements OnInit {
  ...
  checkedUser(teamId:any,userId:any)
  {
      if(this.selectedUserID.length!=0)
      {
        ...
        {
            let indx = this.selectedUserID[teamId].findIndex((obj:any,ind:any)=>{
  ...
  SubmitAllotment(modal:any)
  {
    ...
    Swal.fire({
      title: 'Are you sure?',
      text: 'Do you really want to allot exercise.',
      icon: 'warning',
      ...
      if(result.value)
      {
          if(this.credit_system=='Active')
          {
              ...
              {
                  Swal.fire('',"Sorry! You don't have sufficient credits in wallet.",'warning');
                  return true;
              }
 ...
 TeamCheckboxChange(team_id:string, isChecked: boolean)
 {
  ...
  setAllotmentData()
  {
    ...
    this.current_allot_type = this.userfrm.value.allocation_type;
	  var api = 'admin-api/set-allotment-data';
	  this.BackenddbService.postData(api,formData).subscribe((res:any) => {
          this.ds.Loader(false);
          this.user_data = [];
          this.team_data = [];
  ...
  { 
    if(step==2)
    {
      ...
      {
        Swal.fire('','Sorry! select exercise first!','warning');
        return true;
      }else if(this.userfrm.value.allocation=='Training_Group' && this.train_gp_array.length==0)
      {
        Swal.fire('','Sorry! select training group first!','warning');
        return true;
      }
    ...
    }else if(step==3)
    {
      ...
      {
        Swal.fire('','Sorry! select user first!','warning');
        return true;
      }else if(this.userfrm.value.allocation_type=='Team' && teamFormArray.length==0)
      {
        Swal.fire('','Sorry! select team first!','warning');
        return true;
      }
  ...
  public onItemSelect(item: any,type:any) 
 {
    ...
    }else if(type=="Train_Gp")
    {
     //  this.exercise_array.push(item.id);
      ...
      let result = this.train_gp_dropdownList.find(x => x.id == item.id);

     // let result = this.train_gp_dropdownList[index]
  ...
  public onVMSelect(item: any,type:any) 
  {
    if(type=='Vm')
    {
          ...
          this.user_data.forEach((myObject:any, index:any) => {
            if(typeof this.user_data[index].vm_control !='undefined')
            {
              //if team empty then search for specify user else search for all user
                  this.user_data[index].vm_control.forEach((us_obj:any, ind:any) => {
                        if(this.current_user_id == this.user_data[index].U_ID && this.current_ex_id==this.user_data[index].vm_control[ind].ex_id && this.current_team_id==this.user_data[index].vm_control[ind].team_id && item.unique_id == this.user_data[index].vm_control[ind].unique_id)
                        {
                          this.valid = true;
                        }
                  });   
      ...
      }else
      {
        this.team_data.forEach((teamObj:any, tindex:any) => {
          teamObj.user_data.forEach((usObj:any, index:any) => {
          if(typeof teamObj.user_data[index].vm_control !='undefined')
          {
                 //if team empty then search for specify user else search for all user
                 teamObj.user_data[index].vm_control.forEach((us_obj:any, ind:any) => {
                      if(this.current_user_id == teamObj.user_data[index].U_ID && this.current_ex_id==teamObj.user_data[index].vm_control[ind].ex_id && this.current_team_id==teamObj.user_data[index].vm_control[ind].team_id && item.unique_id == teamObj.user_data[index].vm_control[ind].unique_id)
                      {
                        this.valid = true;
                      }
                });   
      ...
      if(this.valid)
      {
         ...
         Swal.fire("","Sorry! VM already access configured.","warning");
         return true;
      }

      this.current_vm_unique_id = item.unique_id;

      if(this.vm_access.length==0)
      {
        var temp1 = {ex_id:this.current_ex_id,team_id:this.current_team_id,unique_id:item.unique_id};
        this.vm_access.push(temp1);
      }else
      {
         var temp = {ex_id:this.current_ex_id,team_id:this.current_team_id,unique_id:item.unique_id,control:this.vm_access[0].control};
         this.vm_access = [];
    ...
    }else if(type=='Control')
    {
     ...
     if(this.selectedVmItems.length==0)
     {
        ...
        Swal.fire("","Sorry! Please select VM first.","warning");
        return true;
     }

      this.vm_access.forEach((myObject:any, index:any) => {
        if(this.vm_access[index].ex_id==this.current_ex_id && this.vm_access[index].team_id==this.current_team_id && this.vm_access[index].unique_id==this.current_vm_unique_id)
        {
          if(typeof this.vm_access[index].control == 'undefined')
          {
            var tmp:any = [item.id];
            this.vm_access[index].control = tmp;
          }else
          {
             var tmp:any = this.vm_access[index].control;
             let data =tmp.find(ob => ob === item.id);
  ...
  }else if(type=='Manual')
  {
      ...
      this.vm_access.forEach((myObj:any, index:any) => {
        if(this.vm_access[index].ex_id==this.current_ex_id && this.vm_access[index].team_id==this.current_team_id && this.vm_access[index].unique_id==this.current_vm_unique_id)
        {
          if(typeof this.vm_access[index].manual == 'undefined' )
          {
  ...
  public onVMDeSelect(item: any,type:any) {
    if(type=='Vm')
    {
      this.selectedControlItems = [];
      this.vm_access.forEach((myObject:any, index:any) => {
          if(this.vm_access[index].ex_id==this.current_ex_id && this.vm_access[index].team_id==this.current_team_id && this.vm_access[index].unique_id==item.unique_id)
          {
    ...
    {
      this.vm_access.forEach((vmacc_obj:any, index:any) => {
          if(typeof vmacc_obj.control != 'undefined')
          {
             vmacc_obj.control.forEach((con_obj:any, con_index:any) => {
                 if(con_obj==item.id)
                 {
                   this.vm_access[index].control.splice(con_index, 1);
                 }
             });
          }else
          {
  ...
  openVmPopUp(ex_id:any,user_id:any,team_id:any,model:any)
  {
    this.current_vm_dropdown = [];
    this.selectedVmItems = [];
    this.vm_access = [];
    this.exercise_data.forEach((obj:any, index:any) => {
      if(this.exercise_data[index].id==ex_id)
      {
         this.current_vm_dropdown = this.exercise_data[index].vm_dropdown;
         var control = {control:obj.permission};
         this.vm_access.push(control);
    ...
    {
      Swal.fire("","Sorry! Please select team member first.","warning")
      return true
    }else
    {
  ...
  SubmitUserConfig()
  {
    ...
    {
      Swal.fire("","Sorry! Please select control.","warning");
      return true
    }    
    ...
    formData.append('current_user_id',this.current_user_id);
    formData.append('current_team_id',this.current_team_id);
    formData.append('current_allot_type',this.current_allot_type);

		this.BackenddbService.setUserVmControl(formData).subscribe((res:any) => {
          this.ds.Loader(false);
          this.vm_access = [];
          this.manual_sel_items = [];
          if(this.current_allot_type=='Individual')
          ...
          this.user_data.forEach((usObj:any, index:any) => {
            if(usObj.U_ID==user_id)
            {
              usObj.vm_control.forEach((vmObj:any, vm_index:any) => {
                  ...
                  {
                    tmp_control = this.user_data[index].vm_control;
                    tmp_control.splice(vm_index,1);
                    this.setUserVmControl(usObj.U_ID,usObj.team_id,tmp_control);
                }
              });
            }
          });
